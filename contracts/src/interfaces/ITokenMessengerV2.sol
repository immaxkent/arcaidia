...
///      rather than guessed: `depositForBurn` takes no `hookData` and returns
///      nothing (the source-side nonce is not part of the event this function
///      emits — it lives in the CCTP message body itself, which is why
///      correlation to Circle's attestation API happens by transaction hash,
///      not by nonce; see `SettlementReference.sourceTxHash`).
