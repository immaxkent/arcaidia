...
- [ ] **27.1 `schema.graphql`.** `Intent` +`intentVersion: Int!`, `tokenOut: Bytes!`, `targetMinOut: BigInt!`,
...
- [ ] **27.2 Mappings.** `router.ts` v2 event; `factory.ts` → `handleVaultCreated` creates the
...
- [ ] **27.3 `generate-subgraph.ts` v2.** Data sources: router v2, factory, receiver v2 (+ the
      retired receivers as today), template for vaults. `START_BLOCKS_V2` filled by WP-31 (placeholder
      `0` until then — `--check` tolerates unset v2 blocks before deploy). `pnpm subgraph:generate && graph codegen && graph build` green.
- [ ] **27.4 Nest re-seed request.** One file `subgraph/nest/RESEED-v2.md`: ABIs (paths), addresses
...
- [ ] **27.5 Fallback path documented:** deploy the same subgraph to Studio for both chains
...
- [ ] `scripts/generate-subgraph.test.ts` — factory + template sections present, addresses from domain config.
