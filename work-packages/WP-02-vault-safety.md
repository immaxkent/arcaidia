...
- [x] **2.1 Policy parameters** on the vault: `reserveFloorBps`, `maxFillAmount`,
...
- [x] **2.2 Consumed-intent map.** `mapping(bytes32 => bool) consumed` marked **before** the
      transfer (checks-effects-interactions). Agent `nonce` tracked separately.
- [x] **2.3 Liquidity accounting.** `availableLiquidity = liquidBalance - reserveFloor`. A fill that
...
- [x] **2.3b ERC-4626 safety** (DECISIONS.md D1): first-depositor inflation/donation attack,
...
- [x] **2.4 Fee bounds.** `outputAmount = inputAmount - feeAmount`; `feeAmount` must satisfy both
      protocol max and the user's `maxFeeBps` carried in the authorization. Zero-fee and
      max-fee edges both tested.
- [x] **2.5 Pause & guardian.** Paused vault rejects all fills; deposits/withdrawals for LPs
      behave per an explicit, documented decision (recommend: withdrawals allowed, fills blocked).
- [x] **2.6 Bidirectional test matrix.** Every scenario below runs with Ethereum-as-destination
      and with Arc-as-destination, driven by config — not by duplicated test files.
- [x] **2.7 Fuzz/invariant tests.** Foundry invariant: `totalAssets()` never decreases except
