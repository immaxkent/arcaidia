...
- [ ] **30.1 Transfer.** `use-intent.tsx` calls `createIntent` v2 (`tokenOut = 0x0`, `targetMinOut = 0`
...
- [ ] **30.2 Trade intent fields** behind `VITE_TRADE_INTENTS_ENABLED`: `tokenOut` select from
...
- [ ] **30.3 Earn / Create vault.** Fee policy tiers — four fee fields + three threshold fields with
...
- [ ] **30.4 Directory + console.** `use-vaults.ts` from `vaults`/`VaultCreated` (Nest/subgraph),
...
- [ ] **30.5 `use-intent-history.ts` / `use-vault-fills.ts`** read the new columns; docs page lists v2 addresses.
