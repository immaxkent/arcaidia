...
(`GET /v1/telemetry/vault/{chainId}/{vaultAddress}/stream` — `chainId` added while building
WP-18: Arcaidia deploys through CREATE2 with identical salts, so the same vault address already
recurs across chains today, and keying on address alone would let one chain's stream/pairing
apply to the other chain's vault); Relay surface is `pair` / `heartbeat` / `events`.
